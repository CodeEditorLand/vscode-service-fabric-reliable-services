...
export function activate(context: ExtensionContext) {
	...
	console.log(
		'Congratulations, your extension "Service Fabric" is now active!'
	);

	// Registering all of the possible commands for interacting with a Service Fabric Project
	context.subscriptions.push(
		commands.registerCommand("vscode-sf.addSFService", addSFService)
	);
	context.subscriptions.push(
		commands.registerCommand("vscode-sf.buildApplication", buildApplication)
	);
	context.subscriptions.push(
		commands.registerCommand(
			"vscode-sf.cleanJavaApplication",
			cleanJavaApplication
		)
	);
	context.subscriptions.push(
		commands.registerCommand(
			"vscode-sf.createApplication",
			createApplication
		)
	);
	context.subscriptions.push(
		commands.registerCommand(
			"vscode-sf.deployApplication",
			deployApplication
		)
	);
	context.subscriptions.push(
		commands.registerCommand(
			"vscode-sf.publishApplication",
			publishApplication
		)
	);
	context.subscriptions.push(
		commands.registerCommand(
			"vscode-sf.removeApplication",
			removeApplication
		)
	);
	context.subscriptions.push(
		commands.registerCommand(
			"vscode-sf.upgradeApplication",
			upgradeApplication
		)
	);
}
